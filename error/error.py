...
def get_error_msg() -> dict:
    ...
    called_by = str(repr(traceback.extract_tb(exc_traceback).__getitem__(0)).split(',')[-1])
    called_by = called_by.replace('>', '')
    line_and_function = str(repr(traceback.extract_tb(exc_traceback).__getitem__(-1)).split(',')[-1])
