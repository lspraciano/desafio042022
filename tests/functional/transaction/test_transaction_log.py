...
def test_transaction_log_template_with_valid_token(app, client_admin_authenticaded, captured_templates):
...
def test_save_transaction_log_with_valid_json(app, client_admin_authenticaded):
    ...
    assert data[0]['transaction_date_time'] in transaction_logs['logs'][0]['transactions_log_transactions_datetime']
    assert app.config['ADMIN_USER_ID'] == transaction_logs['logs'][0]['transactions_log_id']


def test_save_transaction_log_with_invalid_json_field_transaction_home_bank(app, client_admin_authenticaded):
    data = [{
    ...
    for log in transaction_logs['logs']:
        assert data[0]['transaction_date_time'] not in log['transactions_log_transactions_datetime']
